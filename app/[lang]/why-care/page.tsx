...
// No cross-links inside the advice: this page is civic guidance that stands on
// its own, not a tour of the site's features.
const POWER: IconName[] = ['flag', 'info', 'pin', 'star', 'people'];
const GOOD: IconName[] = ['calendar', 'megaphone', 'wallet', 'phone', 'people', 'shield', 'scales'];
const START: IconName[] = ['people', 'star', 'flag', 'pin', 'megaphone'];
      ...
      <div className="mx-auto max-w-4xl px-4 py-8">
        ...
        <section id="good" aria-labelledby="good-h" className="mt-12 scroll-mt-24">
          ...
          <Reveal className="mt-6">
            <div className="glass rounded-3xl p-5 sm:p-6">
              ...
              <div className="mt-5 flex items-start gap-2.5 rounded-2xl bg-paper-soft p-4 text-sm text-ink-soft">
                <Icon name="info" size={17} className="mt-0.5 shrink-0 text-brand" />
                <span>{tr('whyCare.good.note')}</span>
              </div>
            </div>
          </Reveal>
        </section>
          ...
          <div className="mt-6 grid gap-3">
            {items('whyCare.start.items').map((it, i) => (
              <Reveal key={i} delay={(i % 3) * 50} as="div">
                <div className="flex items-start gap-3.5 rounded-2xl border border-line bg-white p-4">
                  <span className="relative inline-grid h-10 w-10 shrink-0 place-items-center rounded-2xl bg-brand-soft text-brand">
                    <Icon name={START[i] ?? 'check'} size={20} />
        ...
        <Reveal className="mt-12">
          <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-brand to-brand-deep p-6 text-white shadow-glow sm:p-7">
            ...
            {/* Quiet further-reading to sibling civic-education pages - not a
                pitch for the site's tools. */}
            <div className="mt-4 flex flex-wrap gap-3">
              ...
              >
                <Icon name="scales" size={15} /> {tr('nav.rights')}
              </Link>
              <Link
                href="/accountability"
              ...
              >
                <Icon name="megaphone" size={15} /> {tr('nav.accountability')}
              </Link>
            </div>
